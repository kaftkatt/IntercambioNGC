...
ny=260    # number of total gridpoints in y-direction
nyS=30

OB=25 #Define where to start with stratified profile in sponge
...
dy=np.linspace(5,2,nyS)
dy=np.append(dy,np.ones(ny-(nyS*2))*2)
dy=np.append(dy,np.linspace(2,5,nyS))
...
dep=(gaussian(y,y[int(umbral)], 10)*900-Ho)
ho=np.tile(dep,(nx,1))
ho[0,:]=0
ho[-1,:]=0
...
tempN[indzSill-1:]=np.arange(tempN[indzSill-1],11,(11-tempN[indzSill-1])/(nz-indzSill+1))
tempBoN=np.tile(np.tile(tempN,(nx,1)).T,(nt,1,1)).T

#Values of temperature at open boundary SOUTH
indz600=len(z[z<600])
tempS=np.zeros(nz)
tempS[:indz600]=np.arange(16,7,(7-16)/indz600)
tempS[indz600-1:]=np.arange(tempS[indz600-1],5,(5-tempS[indz600-1])/(nz-indz600+1))
...
f = open('dy.bin','wb')
write_with_byte_inversion(f, dy*1000)
f.close()
...
ax.plot(y,ho[1,:],'green',label='Depth [m]')
...
ax2=ax.twinx()
ax2.plot(y,dy,'k',label='Resolution in y', linestyle='dashed')
ax.minorticks_on()
...
caxT=ax.pcolormesh(y[-OB:],-z,temp[5,-OB:,:].T)
ax.set(xlabel='Distance [km]',ylabel='Depth [m]')
