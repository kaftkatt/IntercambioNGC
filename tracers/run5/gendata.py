...
ny=320    # number of total gridpoints in y-direction
nyS=20
...
dy[:70]=np.flip(dyIn[:-10])
dy[70:240]=500
dy[240:320]=dyIn

#dy=np.append(dyIn2,np.ones(320-288)*dyIn[-1])
...
yB=np.cumsum(dy)
dep=(gaussian(yB,yB[int(nyIn2*0.485)], 8000)*880-Ho)
#dep=np.append(depIN,np.ones(320-288)*-Ho)
ho=np.tile(dep,(nx,1))
ho[0,:]=0
ho[-1,:]=0
ho[:,-1]=0
...
trac[:,:int(nyIn2*0.485),indzSill-5:]=1

#Heatflux
Q=np.zeros((nx,ny,nt))
Qin=-83*(gaussian(np.arange(12), 5.5, 2))
Q[:,20:-20,:]=np.tile(Qin,(nx,ny-40,1))
...
ax2=ax.twinx()
ax2.plot(y/1000,trac[20,:,indzSill-2],'purple',label='Tracer concentration', linestyle='dashed')
...
caxT=ax.pcolormesh(y/1000,-z,temp[20,:,:].T)
ax.set(xlabel='Distance [km]',ylabel='Depth [m]')
...
caxT=ax.pcolormesh(y/1000,np.arange(1,13,1),Q[5,:,:].T)
ax.set(xlabel='Distance [km]',ylabel='Depth [m]')
ax.text(0, 1.02, '(c)', fontweight='bold', color='k', 
        transform=ax.transAxes)

ax.set_title(f'Average in time and space {np.mean(Q):.1f} W/m$^2$')
