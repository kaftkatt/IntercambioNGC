...
project='tides'
location='/home/amelia.th/IntercambioNGC/'
run='/run3/'
...
nt=24 #nt=10  #for 3 years with timestep 60 s
dx=0.5
dz=16
...
yB=np.cumsum(dyIn2)
depIN=(gaussian(yB,yB[int(ny2*0.55)], 8000)*880-Ho)
depIN2=(gaussian(yB,yB[int(ny2*0.55)], 8000)*680-Ho)
dep=np.append(depIN,np.flip(depIN2))
ho=np.tile(dep,(nx,1))
ho[0,:]=0
ho[-1,:]=0

#Values of velocities at open boundaries
velB=np.ones((nx,nz,nt),dtype=dtype)*0.005
...
temp=tempS*np.ones((nx,ny,nz),dtype=dtype)
...
f = open('velB.bin','wb')
write_with_byte_inversion(f, velB)
f.close()
...
ax1=ax.twiny()
ax1.plot(tempS,-z,'blue',label='Northern boundary')
ax1.plot(tempS,-z,'red',label='Southern boundary')
ax1.set(xlabel='Temperature [degC]')

ax2=ax.twinx()
ax2.plot(y/1000,trac[indzSill-2,:,20],'purple',label='Tracer concentration', linestyle='dashed')
ax2.plot(y/1000,dy/1000,'k',label='Resolution in y', linestyle='dashed')
ax.minorticks_on()
...
caxT=ax.pcolormesh(y/1000,-z,temp[5,:,:].T)
ax.set(xlabel='Distance [km]',ylabel='Depth [m]')
...
ax.set_title(fr'Average over basin and in time {-np.mean(Q):.1f} W/m$^2$')
cbar= plt.colorbar(caxT)
cbar.set_label(r'Heatflux [W/m$^2$]')
